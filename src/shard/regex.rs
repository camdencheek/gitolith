        ...
        let prs = ranges
            ...
            .map(|prefix_iter| {
                prefix_iter
                    ...
                    .flatten()
                    .map(move |suffix_idx| sa[suffix_idx])
            })
            .collect();
        Box::new(CheckingDocMatchIterator::new(
    ...
    fn push_concat(&mut self, hirs: Vec<Hir>) {
        for child in hirs.into_iter() {
            if let Some(c) = &self.current {
                if let (_, Some(max_size)) = c.size_hint() {
...
fn split_unicode_ranges(class: hir::ClassUnicode) -> Vec<hir::ClassUnicodeRange> {
    const fn to_char_unchecked(c: u32) -> char {
        unsafe { char::from_u32_unchecked(c) }
    }
    const ONE_BYTE_MIN_CHAR: char = to_char_unchecked(0x0000);
    const ONE_BYTE_MAX_CHAR: char = to_char_unchecked(0x007F);
    const TWO_BYTE_MIN_CHAR: char = to_char_unchecked(0x0080);
    const TWO_BYTE_MAX_CHAR: char = to_char_unchecked(0x0007FF);
    const THREE_BYTE_MIN_CHAR: char = to_char_unchecked(0x0800);
    const THREE_BYTE_MAX_CHAR: char = to_char_unchecked(0xFFFF);
    const FOUR_BYTE_MIN_CHAR: char = to_char_unchecked(0x10000);
    const FOUR_BYTE_MAX_CHAR: char = to_char_unchecked(0x10FFFF);
